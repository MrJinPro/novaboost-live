...
export async function loadHomeActivityFeed(streamers: StreamerCardData[]) {
  ...
  const [boostsResult, postsResult] = await Promise.allSettled([
    supabase
      .from("boosts")
  ...
  const boostRows = boostsResult.status === "fulfilled" && !boostsResult.value.error
    ? ((boostsResult.value.data ?? []) as DbBoostRow[])
    : [];
  const postRows = postsResult.status === "fulfilled" && !postsResult.value.error
    ? ((postsResult.value.data ?? []) as DbPostRow[])
    : [];

  const boostItems = boostRows.map((row) => ({
    id: `boost-${row.id}`,
    tone: "boost" as const,
