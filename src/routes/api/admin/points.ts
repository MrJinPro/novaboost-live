...
export const Route = createFileRoute("/api/admin/points")({
  async loader({ context }) {
    ...
    const { userId: _adminId, error, accessLevel } = await requireAdmin(request);
    if (error) return error;

    // Only full admins can manage points — moderators/support cannot
  ...
  async action({ context }) {
    const { request } = context as { request: Request };
    const { userId: adminId, error, accessLevel } = await requireAdmin(request);
    if (error) return error;

    // Only full admins can award/deduct points
    if (accessLevel !== "admin") {
      return jsonResponse({ error: "Доступ запрещён." }, 403);
    }
    ...
    const { error: ledgerError } = await supabaseAdmin
      ...
      .insert({
        ...
        metadata: { admin_id: adminId, manual: true },
        created_at: now,
      });
